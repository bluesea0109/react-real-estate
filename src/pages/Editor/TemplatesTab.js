...
export default function TemplatesTab({ mailoutDetails, handleSave }) {
  ...
  useEffect(() => {
    ...
    newFilteredStencils.forEach((stencil, index) => {
      if (editIntentPath === stencil.intentPath && editTemplateTheme === stencil.templateUuid) {
        newFilteredStencils.splice(index, 1);
        newFilteredStencils.unshift(stencil);
      }
    });
    setFilteredStencils(newFilteredStencils);
  }, [stencilsAvailable, mailoutDetails.created, editIntentPath, publishedTags, editTemplateTheme]);
