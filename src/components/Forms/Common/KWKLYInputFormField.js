...
const isNotDefaultKeyword = str =>
  str.toLowerCase() === 'keyword' && 'Please replace KEYWORD with other phrase';
...
const KWKLYInputFormField = ({ listingType, initialValues, formValues, setFormValues }) => {
  ...
  let currentValue = editable
    ? formValues?.[listingType]?.kwkly
  ...
  if (!editable) {
    return (
      <Input
        ...
        placeholder="KEYWORD"
        tag={popup(
          'Please enter your KWKLY keyword and we will put the keyword into a the KWKLY phrase for you.'
        )}
      >
